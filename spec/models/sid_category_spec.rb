...
RSpec.describe SidCategory, type: :model do
  ...
  context "#business_count" do
    ...
    let!(:category_with_no_businesses) {create(:sid_category, label: "Surf wax", sid_category_id: "2", order: 2)}
  ...
  context "scopes" do
    ...
    describe ".ordered" do
      it "returns the business in the right order" do
        expect(SidCategory.ordered).to eq [category_with_businesses, category_with_no_businesses]
      end
    end
  end
